...
include( YCANB_PLUGIN_DIR . 'includes/shortcodes.php' );
    ...
    if( get_post_type() == 'anb' ) {
		...
		foreach ( $post_types as $post_type => $post_type_name ) {
			...
			if ($post_type_name_atr != null) {
				$check_post_type = (!empty($post_type_name_atr) && !empty($post_type)) ? strpos($post_type_name_atr, $post_type) : false;
		...
		foreach ($anbs as $alert_notice_values) {
			if ( $alert_notice_values->location_id == $anb_location_id ) {
				...
				if( anb_show($alert_notice_id,$get_page_id) && is_showing_post($alert_notice_id) ) {
					...
					if ( get_post_meta( $anb_design, "anb_close_button_position_option_disable", true ) != 'yes' ) {
						$print_page .= '	<span id="close-anb-id-' . $alert_notice_id . '" class="anb-close ' . $anb_close_design . '" tabIndex="0" title="close box button">&#x2715;</span>'.PHP_EOL;
					}
...
function add_individual_control_meta_box() {
	...
	$is_cpt_public = (isset($wp_post_types[$cpt_name])) ? $wp_post_types[$cpt_name]->public : false;
    if( $is_cpt_public ) {
